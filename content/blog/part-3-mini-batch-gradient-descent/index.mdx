...
Althought the size of the batches matters in MBGD, we will stick to size $64$ this time.

![MSE values produced by BGD and MBGD](./iterations.png)

From the picture, we can see that the cost function produced by MBGD is thicker because the values goes up and down, following a zig-zag pattern,
unlike the one produced by BGD which goes down in a straight line.

Even though MBGD was done in $5000$ iterations, the number of parameter updates happened in MBGD is the same with one in BGD.
...
So, the smaller batch sizes will the model reach the convergence point early with the expense of the noise during training.
On the contrary, the larger batch sizes will produce more accurate model with the expense of reaching the convergence point slower.
