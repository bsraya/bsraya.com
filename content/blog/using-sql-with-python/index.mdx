...
In this case, we want `a_user` to be able to create a database.
Thus we're adding a new privilege to it with `ALTER ROLE a_user WITH CREATEDB;`.

![Show Users in PostgreSQL](./check-user.png)

If you accidentally create a user you don't indent to, simply run
...
The command above, with the `a_user` user account, execute and store the tables described in the script into a database called `film_db`.
...
However, storing millions of data in `.db` file can be another work around as well.
...
Since we have all the CSV files that we need, we have to feed the data into `.db` files.

```python:title=convert-csv-to-sqlitedb.py
import sqlite3
import pandas as pd
...
Then we can see that four new `.db` files have been created. To make sure that these `.db` files are legit, simply run

```python:title=test-if-db-files-are-legit.py
...
Do the same with the other db files. You just simply need to change `films` into another table's name.

That's it. We just learned 8 keypoints mentioned in the beginning of the post. Hope you enjoy it.
