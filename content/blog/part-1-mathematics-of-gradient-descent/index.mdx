...
With the green line, which is our regression line, see $x = 0$ then it predicts $y = -3$. 
In reality, when $x = 0$, $y$ should be $0.5$. Meaning that our regression line is very bad at prediction.
...
Clearly, the MSE of our regression line is relatively smaller compared to the previous one.

The question is, **how do we find the best value for $b_0$ and $b_1$ so that the regression line is so close to most data points in the dataset?**

One way, we can find the best values for $b_0$ and $b_1$ by mere guessing.
However, it is going to take a significant amount of time.

Another way is that we can use Calculus to help us to find the best values for $b_0$ and $b_1$ by updating those two values over time.
