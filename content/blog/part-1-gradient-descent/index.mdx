...
Let's see the intercept and the coefficient that `LinearRegression()` produced from our dataset.
We are also want to see how much the MSE is, so let's print that as well.

```python:title=intercept-coefficient-and-mse
print(reg.intercept_, reg.coefficient_)
# [-3.200215] [[0.75291757]]
...
Before we start working on Gradient Descent, we need the following three functions.
...
def sum_err_graph(algorithm, df, iterations):
    plt.figure(figsize=(6,5))
    plt.plot(np.arange(0, iterations), df['sum_error'][:iterations], color = 'r')
    plt.title(f'{algorithm} Cost Function Over {iterations} Iterations')
    plt.legend(["cost funtion"])

def scatter_plot_with_regression_line(gd_intercept, gd_coefficient):
    plt.figure(figsize=(6,5))
    sns.scatterplot(
        x = sepal_length,
        y = petal_width,
    ...
    plt.title(f"intercept = {gd_intercept:.2f} and coefficient = {gd_coefficient:.2f}")

def calculate_mse(algorithm, gd_intercept, gd_coefficient):
    gd_predictions = [gd_intercept + gd_coefficient * x for x in sepal_length]
    gd_mse = mean_squared_error(
        gd_predictions,
        petal_width
    )
    print(f"The {algorithm}'s MSE is {gd_mse}")
```
...
Then we create a dataframe to store the result and run the `gd()` function for over 10000 times.

```python:title=gd-10000-iterations
...
## 1.1 GD 1000 Iterations

```python:title=1000-iter-gd
epoch = 1000
calculate_mse(
    'GD',
