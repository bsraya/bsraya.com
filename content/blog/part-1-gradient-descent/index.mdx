...
There are several equations that we need to familiar with before working on these implementations, namely: **Mean Squared Error**, **the regression line equation**, and **the weight update equation**.
...
First, let's fit our dataset to `py:LinearRegression()` model that we imported from `py:sklearn.linear_model`.
