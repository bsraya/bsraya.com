...
From the MSE equation, we then have a cost function for Gradient Descent which means how far, or the error, the prediction from the regression line.
...
$$
\Theta_{i+1} = \Theta_i - \frac{\alpha}{N} \sum^{N}_{i=1} (\hat{y}_i - y_i) \times x_i
$$
